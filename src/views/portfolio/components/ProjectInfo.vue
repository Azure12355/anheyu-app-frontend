...
import type { GalleryImageItem, Portfolio } from "@/types/portfolio";
...
const galleryImages = computed<GalleryImageItem[]>(() => {
  const rawList = props.portfolio.gallery_images || [];
  return rawList
    .map(item =>
      typeof item === "string"
        ? { image: item, link: "" }
...
const hasRichInfo = computed(() => {
  return (
    props.portfolio.overview ||
    props.portfolio.challenge ||
...
const imageStates = ref<
  Record<number, { loaded: boolean; aspectRatio: number }>
>({});
...
<template>
  <div class="project-info-container">
    ...
    <div class="editorial-grid">
      <!-- Overview -->
      <div
        v-if="portfolio.overview || portfolio.overview_html"
        class="editorial-block full-width"
      >
      ...
      <div
        v-if="
          portfolio.challenge ||
          portfolio.challenge_html ||
          portfolio.solution ||
          portfolio.solution_html
        "
        class="editorial-block two-col"
      >
        <div v-if="portfolio.challenge || portfolio.challenge_html" class="col">
          <h3 class="subsection-heading">
            <IconifyIconOnline icon="ri:focus-3-line" class="heading-icon" />
            所遇挑战
          </h3>
          <MarkdownContent
        ...
        <div v-if="portfolio.solution || portfolio.solution_html" class="col">
          <h3 class="subsection-heading">
            <IconifyIconOnline
    ...
    <div v-if="galleryImages.length > 0" class="project-gallery">
    ...
    <div v-if="!hasRichInfo" class="stats-block">
      <div class="stat-item">
...
.image-overlay {
  ...
  /* 渐变遮罩 */
  background: linear-gradient(to top, rgb(0 0 0 / 30%) 0%, transparent 50%);
  border-radius: 16px;
  opacity: 0;
